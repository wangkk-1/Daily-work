      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="item in cartList" :key="item.id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" :value="item.skuNum" minnum="1" class="itxt" @change="changeItemCount(item,$event.target.value-item.skuNum,$event)" />
...
export default {
  ...
  methods: {
    ...
    //删除某个商品
    async deleteItem (item){console.log(item)
      if(window.confirm(`确定删除${item.skuName}么?`)){
        try {
